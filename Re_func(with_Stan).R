...
#set a time 
t <- seq(0,99999,by=100)
#input the normal use condition x1 is RH, x2 is Temp
x1 <- rep((log(50)-log(40))/(log(85)-log(40)),1000)
head(x1)
x2 <- rep((11605/(25+273.15)-11605/(15+273.15))/(11605/(85+273.15)-11605/(15+273.15)),1000)
head(x2)
...
re_func <- function(t, par, bvn,x1,x2){
  ...
  beta1 <- NULL
  gamma <- NULL
  deg <- matrix(rep(0,N*1000),N,1000)
  for(i in 1:N){
    for(j in 1:1000)
    {
      beta0[i] <- par[i,4]+bvn[i,1]
      beta1[i] <- exp((par[i,3])+par[i,1]*x1[j]+par[i,2]*x2[j])
      gamma[i] <- par[i,5]+bvn[i,2]
      deg[i,j] <- beta0[i]+beta1[i]*(t[j]^(gamma[i]))
    }
  }
  return(list(beta0,deg))
}
sum(deg[,1]>log(280))/dim(deg_prob)[2]
deg<-re_func(t=t,par=sampler_draws,bvn=bvn,x1=x1,x2=x2)
deg_prob<-(deg1[[2]]>log(280))
...
ggplot()+
  geom_smooth(mapping=aes(x=t[1:1000],y=1-deg_new[1:1000]))  

for(i in 1:165){
  beta0[i] <- sampler_draws[i,4]+bvn[i,1]
}